...
public class LauncherImpl extends UnicastRemoteObject implements Launcher {
     ...
     * RMI callable, called by a new client when it arrives. */
    ...
    /** Publish the current state of the group to all the clients and activate them. */
    public void go() {
        try {
    ...
    /** Fetch the data of the clients and print them. */
    public void print() {
    	int numcl = 1;
        for (Client c : clients) {
