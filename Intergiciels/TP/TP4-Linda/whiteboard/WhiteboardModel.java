...
public class WhiteboardModel implements com.ibm.tspaces.Callback {
	...
	public void start(String host, int port) {
		try {
			...
			if (TSServer.read(match) == null) {
				...
				TSServer.write(KEY_WHITEBOARD, Command.LOCK);
			}
			...
			// **** TODO1 ****
			TSServer.eventRegister(TSServer.WRITE, new Tuple(KEY_WHITEBOARD, Command.DRAW, new Field(ColoredShape.class)), this);

			// Same thing for Erase tuples.
			// **** TODO2 ****
			TSServer.eventRegister(TSServer.WRITE, new Tuple(KEY_WHITEBOARD, Command.ERASEALL), this);

			// Same thing for Rotate tuples.
			// **** TODO5 ****
			TSServer.eventRegister(TSServer.WRITE, new Tuple(KEY_WHITEBOARD, Command.ROTATE, new Field(Integer.class)), this);
		...
		// **** TODO4 ****
		try {
			Tuple allDraw = TSServer.scan(KEY_WHITEBOARD, Command.DRAW, new Field(ColoredShape.class));

			for (int i = 0; i < allDraw.numberOfFields(); i++) {
				Tuple tuple = (Tuple) allDraw.getField(i).getValue();
				lines.add((ColoredShape) tuple.getField(2).getValue());
			}
		} catch (TupleSpaceException e) {
			System.out.println("TupleSpaceException " + e);
		}
	...
	public void eraseAll() {
		...
		lines.clear();
		try {
			TSServer.write(KEY_WHITEBOARD, Command.ERASEALL);
			TSServer.deleteAll();
			TSServer.write(KEY_WHITEBOARD, Command.LOCK);
		} catch (TupleSpaceException e) {
			System.out.println("TupleSpaceException " + e);
	...
	public void rotateAll(int degree) {
		System.out.println("Rotate all");
			...
			// **** TODO5 ****
		try {
			TSServer.write(KEY_WHITEBOARD, Command.ROTATE, 45);
		} catch (TupleSpaceException e) {
			System.out.println("TupleSpaceException " + e);
		}
	}
	...
	public void acquireExclusiveAccess() {
		...
		// **** TODO3 ****
		try {
			TSServer.waitToTake(new Tuple(KEY_WHITEBOARD, Command.LOCK));
	...
	public synchronized boolean call(String eventName_, String tsName_, int sequenceNumber_, SuperTuple tuple_, boolean isException_) {
		...
		switch (cmd) {
		  ...
		  case DRAW:
			...
			// **** TODO1 ****
			try {
				ColoredShape shape = (ColoredShape) (tuple_.getField(2).getValue());
				lines.add(shape);
			} catch (TupleSpaceException e) {
				System.out.println("Whiteboard::call(DRAW) TupleSpaceException " + e.getMessage());
				return false;
			}
			break;
		  case ROTATE:
	...
	public void addShape (ColoredShape shape)
	{
		...
		// **** TODO1 ****  and **** TODO3 ****
		try {
			if (!hasExclusiveAccess && TSServer.read(KEY_WHITEBOARD, Command.LOCK) == null) {
				return;
			}

			lines.add(shape);
		} catch (TupleSpaceException e) {
			System.out.println("TupleSpaceException " + e.getMessage());
		}
