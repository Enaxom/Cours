...
I = double(I);
I_decorr = diff(transpose(I));
I_decorr = [I(:,1) transpose(I_decorr)];
...
histogramme = hist(I_decorr(:),[I_min:I_max]);
...
I_gauche = I_decorr(:,1:nb_colonnes-1);
I_gauche = I_gauche(:);
I_droite = I_decorr(:,2:nb_colonnes);
I_droite = I_droite(:);
