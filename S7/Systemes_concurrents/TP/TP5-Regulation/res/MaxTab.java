...
class TraiterFragment extends RecursiveTask<Integer> {
	...
	protected Integer compute() {
		...
		if (fin-début > TraiterFragment.seuil) {
			int mid = début + (fin - début) / 2;

			TraiterFragment st1 = new TraiterFragment(tableau, début, mid);
			TraiterFragment st2 = new TraiterFragment(tableau, mid + 1, fin);

			st1.fork();
			st2.fork();

			max = st1.compute();
			max = Math.max(st2.join(), max);
		} else {
			for (int i = début; i <= fin; i++) {
