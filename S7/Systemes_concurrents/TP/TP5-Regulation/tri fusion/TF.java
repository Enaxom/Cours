...
class TraiterFragment extends RecursiveTask<int[]> {
	...
	protected int[] compute() {
		...
		if (fin - début > TF.seuil) {
			int mid = début + (fin - début) / 2;

			TraiterFragment sf1 = new TraiterFragment(tableau, début, mid);
			TraiterFragment sf2 = new TraiterFragment(tableau, mid + 1, fin);

			sf1.fork();
			sf2.fork();

			resTri = sf1.compute();
			resTri = TF.fusion(resTri, sf2.join());
		} else {
...
public class TF {
	...
	throws InterruptedException, ExecutionException {
		...
		// Je ne vois pas comment utiliser cette liste
		// List<int[]> resTri = new LinkedList<int[]>(); //recueille et fusionne les résultats

		//soumettre les tâches
		for (int i = 0; i < nbT; i++) {
			d = i + grain;
			f = (i == nbT - 1) ? t.length - 1 : d + grain - 1;

			résultats.add(xs.submit(new TriLocal(t, d, f)));
		}

		//récupérer et fusionner les résultats.
		// Tableau trié, d'abord égal à la première partie triée puis concaténé
		int[] resTri = résultats.get(0).get();

		for (int i = 1; i < nbT; i++) {
			resTri = TF.fusion(resTri, résultats.get(i).get());
		}
