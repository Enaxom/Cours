...
public class IncrMes {
	...
	// Question 9
	//static AtomicLong cptAtomic = new AtomicLong(0L);
 ...
 */
	public void incr() {
		System.out.println("Compteur avant : " + IncrMes.cpt);
		for (long i = 0L; i < IncrMes.NB_IT; i++) {
			...
			//synchronized (IncrMes.mutex) {
			for (long j = 1; j<=IncrMes.NB_IT_INTERNES; j++) {
				// synchronized (IncrMes.mutex) {
					// Question 9
					//IncrMes.cptAtomic.incrementAndGet();
					//IncrMes.cpt=IncrMes.cpt+j/j;
					IncrMes.cpt=IncrMes.cpt++;
				// }
			}
			//}
			try {
		...
		System.out.println("Compteur après : " + IncrMes.cpt);
	}
